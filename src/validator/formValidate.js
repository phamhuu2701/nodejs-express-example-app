 ...
 * @param {string} type "text" | "number" | "email" | "tel"
 */
module.exports.formValidate = (value, options, type = 'text') => {
  try {
    // check options
    ...
    if (
      ...
      return VALIDATE_RESPONSE.error(
        VALILDATE_ERROR.minlength(value, options.minlength),
      );
    }
    if (
      options.maxlength &&
      ...
      return VALIDATE_RESPONSE.error(
        VALILDATE_ERROR.maxlength(value, options.maxlength),
      );
    }
    if (
      options.pattern &&
